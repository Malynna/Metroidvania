...
func _physics_process(delta):
	...
	match state:
		MOVE:
			...
			apply_horizonatal_force(input_vector, delta)
			apply_fraction(input_vector)
			update_snap_vector()
			jump_check()
			apply_gravity(delta)
...
func apply_horizonatal_force(input_vector, delta):
	if input_vector.x != 0:
		motion.x += input_vector.x * ACCELERATION * delta
		motion.x = clamp(motion.x, -MAX_SPEED, MAX_SPEED)


func apply_fraction(input_vector):
...
func jump(force):
	SoundFX.play("Jump", rand_range(0.8, 1.0), -10)
...
func move():
	...
	if is_on_floor() and get_floor_velocity().length() == 0 and abs(motion.x) < 0:
